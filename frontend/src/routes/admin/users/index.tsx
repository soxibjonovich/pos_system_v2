...
function RouteComponent() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      // Filter only operational users
      const staffUsers = usersList.filter(
        (user: User) => user.role === "staff" || user.role === "chef",
      );
  ...
  const addUser = async (data: UserFormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.detail || `Failed to add user: ${response.statusText}`,
        );
      }
  ...
  const staffRoles = ["staff", "chef", "admin"];
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Staff & Chef Users</h1>
      ...
      <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new user account (staff/chef). PIN must be 4-6 digits.
            </DialogDescription>
          </DialogHeader>
