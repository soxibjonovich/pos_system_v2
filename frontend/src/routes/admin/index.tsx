...
function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const [todaySales, yesterdaySales, inventory, orders, users, topProds] =
        await Promise.all([
          fetchJson(
            `${API_URL}${api.admin.base}/${api.admin.reports}/sales?${todayParams}`,
          ),

          fetchJson(
            `${API_URL}${api.admin.base}/${api.admin.reports}/sales?${yesterdayParams}`,
          ),

          fetchJson(
            `${API_URL}${api.admin.base}/${api.admin.reports}/inventory`,
          ),

          fetchJson(`${API_URL}${api.orders.base}/${api.orders.orders}`),

          fetchJson(`${API_URL}${api.admin.base}/${api.admin.users}`),
          ...
          (() => {
            ...
            return fetchJson(
              `${API_URL}${api.admin.base}/${api.admin.reports}/sales?${params}`,
            );
          })(),
        ]);
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded text-xs font-semibold ${badges[status] || badges.pending}`}
      >
        {t(`status.${status}`)}
      </span>
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatPrice(salesStats.today)}
            </div>
            <div
              className={`flex items-center gap-1 text-xs mt-1 ${getTrendColor(salesStats.trend)}`}
            >
              {getTrendIcon(salesStats.trend)}
              <span>{Math.abs(salesStats.trend).toFixed(1)}%</span>
              <span className="text-muted-foreground">
                {t("dashboard.trend.vsYesterday")}
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            ...
            <div className="flex gap-3 text-xs mt-1 text-muted-foreground">
              <span className="text-yellow-600">
                {orderStats.pendingOrders} {t("dashboard.stats.pending")}
              </span>
              <span className="text-green-600">
                {orderStats.completedToday} {t("dashboard.stats.completed")}
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            ...
            <div className="flex gap-3 text-xs mt-1">
              ...
              <span className="text-red-600">
                {inventoryStats.outOfStock} {t("dashboard.stats.outOfStock")}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex gap-3 text-xs mt-1 text-muted-foreground">
              <span>
                {userStats.activeUsers} {t("dashboard.userStatus.active")}
              </span>
              <span>
                {userStats.staffCount} {t("dashboard.stats.users")}
              </span>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t(`dashboard.stats.lastSevenDays`)}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {salesTrend.length > 0 ? (
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              {t("dashboard.stats.lastFiveOrders")}
            </CardDescription>
          </div>
                ...
                <thead className="bg-muted">
                  <tr>
                    ...
                    <th className="text-left p-3 text-sm font-semibold">
                      {t("dashboard.order.time")}
                    </th>
                    <th className="text-left p-3 text-sm font-semibold">
                      {t("dashboard.order.table")}
                    </th>
                    <th className="text-left p-3 text-sm font-semibold">
                      {t("dashboard.order.server")}
                    </th>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <Link to="/admin/users">
              <Button variant="outline" className="w-full h-20 flex-col gap-2">
                ...
                <span className="text-xs">
                  {t("dashboard.action.manageUsers")}
                </span>
              </Button>
            </Link>
