...
async def get_sales_summary(
    ...
    try:
        ...
        summary_resp = await service_client.db_client.get("/reports/sales/summary", params=params)
        top_products_resp = await service_client.db_client.get("/reports/sales/top-products", params=params)
        by_day_resp = await service_client.db_client.get("/reports/sales/by-day", params=params)
        by_hour_resp = await service_client.db_client.get("/reports/sales/by-hour", params=params)
        
        if summary_resp.status_code != 200:
            raise Exception("Failed to fetch sales summary")
        
        summary = summary_resp.json()
        top_products = top_products_resp.json() if top_products_resp.status_code == 200 else []
        ...
        return schemas.SalesSummaryResponse(
            ...
            sales_by_day=[
                schemas.SalesByDay(date=d['date'], total=d['total'])
                for d in by_day
            ],
            sales_by_hour=[
                schemas.SalesByHour(hour=h['hour'], total=h['total'])
                for h in by_hour
            ]
        )
...
async def generate_excel_report(
    ...
    try:
        ...
        # Date range
        date_format = "%d.%m.%Y"
        ws_summary['A2'] = f"Period: {start_date.strftime(date_format) if start_date else 'All'} - {end_date.strftime(date_format) if end_date else 'All'}"
        ...
        for cell in ['A1', 'B1', 'C1']:
            ...
            ws_products[cell].fill = PatternFill(start_color="4472C4", end_color="4472C4", fill_type="solid")
        ...
        for cell in ['A1', 'B1']:
            ...
            ws_daily[cell].fill = PatternFill(start_color="4472C4", end_color="4472C4", fill_type="solid")
        ...
        if len(summary.sales_by_day) > 0:
            ...
            data = Reference(ws_daily, min_col=2, min_row=1, max_row=len(summary.sales_by_day) + 1)
            cats = Reference(ws_daily, min_col=1, min_row=2, max_row=len(summary.sales_by_day) + 1)
