...
class ServiceClient:
    def __init__(self)->None:
        ...
        self.auth_client=httpx.AsyncClient(base_url="http://127.0.0.1:8003",timeout=10.0)
...
def handle_service_errors(func:Callable)->Callable:
    ...
    async def wrapper(*args:Any,**kwargs:Any)->Any:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_504_GATEWAY_TIMEOUT,detail="Database service timeout")
...
@handle_service_errors
async def update_order(order_id:int,order:schemas.OrderUpdate)->schemas.OrderResponse|None:
    response=await service_client.db_client.put(f"/orders/{order_id}",json=order.model_dump(exclude_unset=True))
    if response.status_code==404:
        return None
    if response.status_code==200:
        return schemas.OrderResponse.model_validate(response.json())
...
@handle_service_errors
async def update_order_status(order_id:int,new_status:str)->schemas.OrderResponse|None:
    response=await service_client.db_client.patch(f"/orders/{order_id}/status",json={"status":new_status})
    if response.status_code==404:
        return None
    if response.status_code==400:
...
@handle_service_errors
async def add_order_item(order_id:int,item:schemas.OrderItemCreate)->schemas.OrderResponse|None:
    response=await service_client.db_client.post(f"/orders/{order_id}/items",json=item.model_dump())
    if response.status_code==404:
...
@handle_service_errors
async def update_order_item(order_id:int,item_id:int,item:schemas.OrderItemUpdate)->schemas.OrderResponse|None:
    response=await service_client.db_client.put(f"/orders/{order_id}/items/{item_id}",json=item.model_dump(exclude_unset=True))
    if response.status_code==404:
        return None
    if response.status_code==200:
        return schemas.OrderResponse.model_validate(response.json())
...
@handle_service_errors
async def remove_order_item(order_id:int,item_id:int)->schemas.OrderResponse|None:
    response=await service_client.db_client.delete(f"/orders/{order_id}/items/{item_id}")
    if response.status_code==404:
        return None
    if response.status_code==200:
        return schemas.OrderResponse.model_validate(response.json())
