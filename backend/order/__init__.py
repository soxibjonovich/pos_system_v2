...
mapp.add_middleware(CORSMiddleware,allow_origins=["*"],allow_credentials=True,allow_methods=["*"],allow_headers=["*"])
...
async def get_config():
    ...
    return schemas.SystemConfigResponse(business_type=config)

@app.get("/tables",response_model=schemas.TablesResponse)
async def get_tables():
    tables=await crud.get_active_tables()
    return schemas.TablesResponse(tables=tables,total=len(tables))
...
@app.get("/{order_id}",response_model=schemas.OrderResponse,status_code=status.HTTP_200_OK)
...
async def get_orders_by_status(order_status:str,_:schemas.User=Depends(get_current_staff)):
...
async def update_order(order_id:int,order:schemas.OrderUpdate,_:schemas.User=Depends(get_current_staff)):
...
async def update_order_status(order_id:int,status_update:schemas.OrderStatusUpdate,_:schemas.User=Depends(get_current_staff)):
...
async def add_order_item(order_id:int,item:schemas.OrderItemCreate,_:schemas.User=Depends(get_current_staff)):
...
async def update_order_item(order_id:int,item_id:int,item:schemas.OrderItemUpdate,_:schemas.User=Depends(get_current_staff)):
    updated_order=await crud.update_order_item(order_id,item_id,item)
    if not updated_order:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Order or item not found")
    return updated_order

@app.delete("/{order_id}/items/{item_id}",response_model=schemas.OrderResponse)
async def remove_order_item(order_id:int,item_id:int,_:schemas.User=Depends(get_current_staff)):
...
async def get_order_total(order_id:int,_:schemas.User=Depends(get_current_staff)):
    ...
    if not order:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Order not found")
    return schemas.OrderTotalResponse(order_id=order.id,total=order.total,items_count=len(order.items))
...
async def missing_token_handler(request:Request,exc:MissingTokenError):
    return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED,content={"detail":"Missing authentication token"},headers={"WWW-Authenticate":"Bearer"})

@mapp.exception_handler(AuthXException)
async def authx_exception_handler(request:Request,exc:AuthXException):
    return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED,content={"detail":"Invalid or expired token"},headers={"WWW-Authenticate":"Bearer"})

@mapp.exception_handler(NoAuthorizationError)
async def no_authorization_handler(request:Request,exc:NoAuthorizationError):
    return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED,content={"detail":"Authentication required"},headers={"WWW-Authenticate":"Bearer"})
