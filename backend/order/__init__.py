...
from deps import UserRole, get_current_staff, require_roles
from fastapi import (
    APIRouter,
    Depends,
    FastAPI,
...
mapp = FastAPI(
    title="Order Microservice", version="1.0", lifespan=lifespan, root_path="/api/order"
)
mapp.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def get_config():
    ...
    qqs_percent = await crud.get_qqs_percent()
...
async def get_orders(_: schemas.User = Depends(get_order_viewer)):
...
async def get_orders_by_status(
    order_status: str, _: schemas.User = Depends(get_order_viewer)
):
...
async def create_order(order: schemas.OrderCreate):
    ...
    if business_type == "restaurant" and order.table_id is None:
        raise HTTPException(
            ...
            detail="Table required for restaurant orders",
        )
