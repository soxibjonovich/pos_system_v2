...
class ServiceClient:
    def __init__(self):
        self.client = httpx.AsyncClient(base_url="http://127.0.0.1:8003", timeout=10.0)
...
async def get_products() -> schema.ProductsResponse:
    try:
        ...
        data = response.json()
        return schema.ProductsResponse.model_validate_json(data)
...
async def create_product(product: schema.ProductCreate) -> schema.ProductResponse | None:
    try:
...
async def update_product(product_id: int, product: schema.ProductUpdate) -> schema.ProductResponse | None:
    try:
        response = await service_client.client.put(
            f"/products/{product_id}",
            json=product.model_dump(exclude_unset=True)
        )
