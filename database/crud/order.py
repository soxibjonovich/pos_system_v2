...
async def update_order(db: AsyncSession, order_id: int, order_data: schema.OrderUpdate) -> Order | None:
...
async def add_order_item(db: AsyncSession, order_id: int, item_data: schema.OrderItemCreate) -> Order | None:
    order = await get_order_by_id(db, order_id)
...
async def update_order_item(
    ...
    order_id: int, 
    item_id: int, 
    item_data: schema.OrderItemUpdate
) -> Order | None:
    order = await get_order_by_id(db, order_id)
...
async def remove_order_item(db: AsyncSession, order_id: int, item_id: int) -> Order | None:
    order = await get_order_by_id(db, order_id)
